...
gs4_auth(
  email = "longevitylabs23@gmail.com",
  path = NULL,
...
sheet_id <- drive_get("Audiencetool")$id
#Read our sheet
...
server <- function(input, output, session) {
  modal_open <- reactiveVal(FALSE) # Tracks if the modal is open
  
  observeEvent(input$submit, {
    missing_inputs <- c(
      if (is.null(input$albumin) || is.na(input$albumin) || input$albumin == "") "Albumin",
      if (is.null(input$lymph) || is.na(input$lymph) || input$lymph == "") "Lymphocytes",
      if (is.null(input$mcv) || is.na(input$mcv) || input$mcv == "") "Mean Cell Volume",
      if (is.null(input$glucose) || is.na(input$glucose) || input$glucose == "") "Glucose",
      if (is.null(input$rdw) || is.na(input$rdw) || input$rdw == "") "Red Cell Distribution Width",
      if (is.null(input$creat) || is.na(input$creat) || input$creat == "") "Creatinine",
      if (is.null(input$crp) || is.na(input$crp) || input$crp == "") "CRP"
    )
    
    missing_inputs <- missing_inputs[!is.null(missing_inputs)]
    
    if (length(missing_inputs) > 0) {
      # Show notification for missing inputs
      showNotification(
        paste("Please fill in the following fields:", paste(missing_inputs, collapse = ", ")),
        type = "error"
      )
      return(NULL) # Stop execution if inputs are missing
    }
    
    if (!modal_open()) { # Trigger modal only if it hasn't been shown yet
      removeModal() # Remove any residual modal if present
      albumin <- input$albumin
      lymphocytes <- input$lymph
      mean_cell_volume <- input$mcv
      glucose <- input$glucose
      red_cell_dist_width <- input$rdw
  ...
  observe({
    ...
    iv$add_rule("bloodTestDate", sv_required("Date of Test is required"))
    iv$add_rule("albumin", sv_required("Albumin value is required"))
    iv$add_rule("lymph", sv_required("Lymphocytes value is required"))
    iv$add_rule("mcv", sv_required("Mean Cell Volume value is required"))
    iv$add_rule("glucose", sv_required("Glucose value is required"))
    iv$add_rule("rdw", sv_required("Red Cell Dist Width value is required"))
    iv$add_rule("creat", sv_required("Creatinine value is required"))
    iv$add_rule("crp", sv_required("CRP value is required"))
  ...
  observe({
    shinyjs::disable("submit") # Disable submit button by default
    phone_number <- input$phone_number
    if (nchar(phone_number) == 10 && !grepl("[^0-9]", phone_number)) {
      shinyjs::enable("submit") # Enable submit button if phone number is 10 digits and contains only numbers
    }
  })
  ...
  observeEvent(input$submit, {
    ...
    if(nrow(allInputs_tosave)==1){
      ...
      if (nrow(values) == 0) {
        sheet_write(data = allInputs_tosave,
                    ss = sheet_id,
                    sheet = "longevity")
      } else {
        sheet_append(data = allInputs_tosave,
                     ss = sheet_id,
                     sheet = "longevity")
      }
    }
